...
export default function VisualizeRoutePage() {
  ...
  useEffect(() => {
    fetch('/api/users/list')
      ...
      .then((data: ListUser[]) => setUsers(Array.isArray(data) ? data : []))
      .catch(() => setUsers([]));
  }, []);
    ...
    <MapProvider>
      <div className="h-screen flex flex-col p-4">
        ...
        <div className="mb-4 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg shadow">
          {canSelectUser && (
            <div className="mb-4">
              <label htmlFor="route-user" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                User
              </label>
              <select
                id="route-user"
        ...
        {selectedSince && !mapInstance && (
          <p className="text-orange-500 p-2">Map instance not yet available for RouteVisualizer.</p>
        )}
      </div>
    </MapProvider>
  );
