          ...
          >
            ...
            <div className="p-3">
              <div className="flex justify-between items-center mb-3">
                ...
                <div className="text-white text-base font-medium">{`${monthName} ${currentMonth.getFullYear()}`}</div>
                <button
                  onClick={nextMonth}
                  className="text-white bg-zinc-700 hover:bg-zinc-600 p-2 rounded-full touch-manipulation"
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isSelected ? "bg-lime-600 font-bold" : "hover:bg-zinc-700"
                      )}
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isSelected ? "bg-lime-600 font-bold" : "bg-zinc-700 hover:bg-zinc-600"
                      )}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <div className="p-6 space-y-4">
          ...
          <Button onClick={onClose} className="w-full h-[3.5rem] text-lg" variant="outline">
            Close
          </Button>
        </div>
      </div>
    ...
    >
      ...
      >
        <div className="bg-slate-900 p-4 flex justify-between items-center border-b border-lime-800">
          ...
          <Button onClick={onClose} variant="ghost" size="icon" className="text-white hover:bg-slate-700">
            <X className="h-6 w-6" />
            <span className="sr-only">Close</span>
          </Button>
        </div>
        <div className="flex-1 overflow-auto p-4 bg-slate-900">{children}</div>
      </div>
    </div>,
    document.body
  )
...
function calendarAppointmentToRawGCalEvent(
...
export function AdjusterForm({
  ...
  const saveAppointment = async () => {
    ...
    try {
      ...
      if (
        watchedValues.adjusterAppointmentDate &&
        watchedValues.adjusterAppointmentTime
      ) {
        ...
        // Build local start/end Date objects
        const startDateTime = new Date(year, month - 1, day, hours, minutes)
        const endDateTime = new Date(year, month - 1, day, hours + 1, minutes)

        // Format local-time strings (no timezone offset)
        const localStart = format(startDateTime, "yyyy-MM-dd'T'HH:mm:ss")
        const localEnd = format(endDateTime, "yyyy-MM-dd'T'HH:mm:ss")

        const payload = {
          summary: `Adjuster Appointment${
            watchedValues.insuranceAdjusterName
              ? ` with ${watchedValues.insuranceAdjusterName}`
              : ""
          }`,
          description: `Adjuster: ${
            watchedValues.insuranceAdjusterName || "TBD"
          }\nPhone: ${watchedValues.insuranceAdjusterPhone || "N/A"}\nEmail: ${
            watchedValues.insuranceAdjusterEmail || "N/A"
          }\n\nNotes: ${
            watchedValues.adjusterAppointmentNotes || "None"
          }`,
          startTime: localStart, // e.g. "2025-06-06T10:00:00"
          endTime: localEnd, // e.g. "2025-06-06T11:00:00"
        ...
        toast({
          ...
          description: `Adjuster appointment scheduled for ${format(
            startDateTime,
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 w-full relative p-1">
        ...
        <div className="space-y-1 sm:space-y-2">
          <div className="flex items-center justify-between">
            <Label
              htmlFor="adjusterAppointmentDate"
              className="text-white text-opacity-90 text-sm sm:text-base"
            >
          ...
          <div className="pt-2 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button
                ...
                disabled={
                  isLoading ||
                  !watch("adjusterAppointmentDate") ||
                  !watch("adjusterAppointmentTime")
                }
